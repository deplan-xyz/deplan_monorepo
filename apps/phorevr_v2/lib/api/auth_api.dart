...
class _AuthApi extends BaseApi {
  ...
  Future<Response> signinEmail(String email, String password) async {
    final entropy = CryptoUtils.generateEntropy();
    final keypair = await CryptoUtils.generateKeypair(entropy);
    _keyPair = keypair;
    final response = await client.post(
      '/phorevr/auth/signin/basic',
      data: {
        'username': email,
        'password': password,
      },
  ...
  Future<Response> signupEmail(String email, String password) async {
    final entropy = CryptoUtils.generateEntropy();
    final keypair = await CryptoUtils.generateKeypair(entropy);
    _keyPair = keypair;
    final response = await client.post(
      '/phorevr/auth/signup/basic',
      data: {
        'wallet': keypair.address,
        'username': email,
        'password': password,
  ...
  Future<Uint8List> encrypt(Uint8List data) async {
    final sk = await CryptoUtils.getSecretKey(_keyPair!);
